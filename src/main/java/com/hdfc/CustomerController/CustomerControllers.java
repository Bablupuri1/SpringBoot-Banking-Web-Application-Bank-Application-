...
/**
 * CustomerControllers is a REST controller that handles all customer-related API requests.
 * Base URL: /Customer_Api
 */
...
public class CustomerControllers {
    ...
    @ResponseBody
    public List<TransactionResponseDTO> getTransactionsByMonthRange(
    ...
    // ------------------------------------------------------------------------------------------------------------
    // GET: Transaction history by date-time range (HTML input type="datetime-local")
    // Example format: 2025-08-24T00:00
    //this is used in admin controller which is used to show the account specific transaction ok
    // ------------------------------------------------------------------------------------------------------------
    @GetMapping("/getTransactionBetweenRange")
    public List<TransactionResponseHistroyDTO> getTransactionHistory(
            @RequestParam String accountNumber,
    ...
    // ------------------------------------------------------------------------------------------------------------
    // GET: All transactions by account number prefect working response************ use showing for netbanikng page when use login
    // ------------------------------------------------------------------------------------------------------------
    ...
    public ResponseEntity<ApiResponse<List<TransactionResponseHistroyDTO>>> getTransactionsByAccountNumber(
        ...
        if (transactions == null || transactions.isEmpty()) {
            ApiResponse<List<TransactionResponseHistroyDTO>> response =
                    new ApiResponse<>(false, "No transactions found for account " + accountNumber, transactions);
